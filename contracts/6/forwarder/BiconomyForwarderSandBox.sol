...
/**
 *
 * @title BiconomyForwarderSandBox
 *
 ...
 *
 */
contract BiconomyForwarderSandBox is ERC20ForwardRequestTypes,Ownable{
    ...
    bytes32 public constant REQUEST_TYPEHASH = keccak256(bytes("ForwardRequest(string info,string action,ERC20ForwardRequest request)ERC20ForwardRequest(address from,address to,address token,uint256 txGas,uint256 tokenGasPrice,uint256 batchId,uint256 batchNonce,uint256 deadline,bytes data)"));

    bytes32 public constant FORWARD_TYPEHASH = keccak256(bytes("ERC20ForwardRequest(address from,address to,address token,uint256 txGas,uint256 tokenGasPrice,uint256 batchId,uint256 batchNonce,uint256 deadline,bytes data)"));
    ...
    function hashForwardRequest(ForwardRequest calldata req) internal view returns (bytes32) {
        return keccak256(abi.encode(
            REQUEST_TYPEHASH,
