...
contract RelayHub is Ownable(msg.sender), EIP712MetaTx("Biconomy Meta Transaction","1") {
	 ...
	 */
	function forward(address[] memory addressArray, bytes memory data, uint256[] memory uintArray,
		bytes32 r, bytes32 s, uint8 v) public onlyProxyOwner(addressArray[0], addressArray[2]) onlyRelayer {

		if(uintArray[0] > 0 && block.number > uintArray[0]) {
			revert("Transaction can not be executed after expiry time");
		}
		require(verifyMetaTxSignature(addressArray, data, uintArray, v, r, s),
		...
		proxyManager.forward(proxyAddress, destinationAddress, uintArray[3], data, uintArray[1], uintArray[4]);
	 ...
	 * @param data data to be executed in meta transaction
	 **/
	function withdraw(address[] memory addressArray, bytes memory data, uint256[] memory uintArray,
		bytes32 r, bytes32 s, uint8 v) public onlyProxyOwner(addressArray[0], addressArray[2]) {

		if(uintArray[0] > 0 && block.number > uintArray[0]) {
			revert("Transaction could not be executed before expiry time");
		}
		address userContract = proxyManager.getProxyAddress(addressArray[0]);
		address payable proxyAddress = address(uint160(userContract));

		require(verifyMetaTxSignature(addressArray, data, uintArray, v, r, s), "Signature does not match with signer");
		address payable receiverAddress = address(uint160(addressArray[1]));
		proxyManager.withdraw(proxyAddress, receiverAddress, uintArray[3], uintArray[4]);
	}
	 ...
	 * @param uintArray array of all param of uint256 type
	 **/
	function verifyMetaTxSignature(address[] memory addressArray, bytes memory data, uint256[] memory uintArray,
		uint8 v, bytes32 r, bytes32 s) public view returns(bool) {

		address payable proxyAddress = address(uint160(proxyManager.getProxyAddress(addressArray[0])));
		...
		EIP712MetaTx.RelayerPayment memory relayerPayment = EIP712MetaTx.RelayerPayment({
			token: addressArray[3],
			amount: uintArray[5]
		});
	...
	function verify(address signer, MetaTransaction memory metaTx, uint8 sigV, bytes32 sigR, bytes32 sigS)
