...
describe("Executor Manager", function () {
  ...
  before(async function () {
    ...
    executor2 = await accounts[2].getAddress();
    notOwner = await accounts[3].getAddress();
  ...
  describe("Test Executor manager methods", function () {
    it("Should addExecutors array successfully", async () => {
      await executorManager.addExecutors([executor1, executor2], {
        from: owner,
      });
      let executors = await executorManager.getAllExecutors();
      expect(executors).to.deep.equal([executor1, executor2]);
    });

    it("Should fail to addExecutors - only owner can perform ", async () => {
      await expect(
        executorManager.addExecutors([executor1], { from: notOwner })
      ).to.be.reverted;
    });

    it("Should removeExecutors array successfully", async () => {
      await executorManager.removeExecutors([executor1, executor2], {
        from: owner,
      });
      let executorStatus = await executorManager.getExecutorStatus(executor1);
      expect(executorStatus).to.equal(false);
      executorStatus = await executorManager.getExecutorStatus(executor2);
      expect(executorStatus).to.equal(false);
    });

    it("Should fail to removeExecutors - only owner can perform ", async () => {
      await expect(
        executorManager.removeExecutors([executor1], { from: notOwner })
      ).to.be.reverted;
    });
    ...
    it("Should fail to addExecutor - only owner can perform ", async () => {
      await expect(executorManager.addExecutor(executor1, { from: notOwner }))
        .to.be.reverted;
    });
    ...
    it("Should fail to removeExecutor - only owner can perform ", async () => {
      await expect(
        executorManager.removeExecutor(executor1, { from: notOwner })
      ).to.be.reverted;
    });
  });
});
